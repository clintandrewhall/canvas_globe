...
export const to3dLocation = (coords, radius) => {
  ...
  return [radius * cosLat * sinLon, radius * sinLat, radius * cosLat * cosLon];
};

export const midpoint = (coord1, coord2, percent) => {
  var midpoint = {
    coordinates: {
      lat: coord1.lat + (coord2.lat - coord1.lat) * percent,
      lon: coord1.lon + (coord2.lon - coord1.lon) * percent,
    },
  };
  return midpoint;
};
